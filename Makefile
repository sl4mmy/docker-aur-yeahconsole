# Copyright (c) 2020 Kent R. Spillner <kspillner@acm.org>
#
...
all: pkg/yeahconsole-$(VERSION)-$(BUILD_NUMBER)-x86_64.pkg.tar.xz

pkg/yeahconsole-$(VERSION)-$(BUILD_NUMBER)-x86_64.pkg.tar.xz: pkg/ build run
...
update: Dockerfile.in
	git subtree pull --prefix $(NAME) https://aur.archlinux.org/yeahconsole.git master --squash
...
.PHONY: all attach build clean run update
